...
* This ADR corrects the earlier "invalidate the manager cache" framing and records the per-path model that lives in the **design corpus**, in that corpus's own revocation design and its NFR document, with evidence record R15. Its build-task and propagation-test numbers are the corpus's and resolve to nothing here; this repository lists the same obligations by what they assert, in the testing design. A bare number cannot be carried between the two, because the same digits name different documents in each.
