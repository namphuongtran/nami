...
* EF Core facts verified: there is no multi-tenant migrate; Microsoft discourages a startup Migrate (concurrency, elevated permissions, no rollback; "consider generating SQL scripts"); `dotnet ef migrations bundle` and `--idempotent` both check `__EFMigrationsHistory`; Finbuckle leaves migration to the developer by its own documentation ("migration responsibility falls to developer"); the ABP DB migrator skips module-specific per-tenant databases (its issue #17977). Expand/contract is Fowler's parallel-change combined with evolutionary database design, and ordered ring rollout follows documented multi-tenant rollout practice (for example the Bytebase and Northflank patterns). The sources and URLs are collected in research record R14, part B.
...
* Bad, because Finbuckle is only connection plumbing (migration is left to the developer) and the ABP DB migrator skips module-specific per-tenant databases (its issue #17977), and neither provides ordered rollout, a traffic-gate, or fleet rollback.
...
* Original decision: 2026-06-29, extended twice by final review on 2026-07-05: finding F10 added the deprovision saga, and finding F48 added tenant move (re-parent and Pool to Silo re-home). This is the tenant-lifecycle ADR and is distinct from the delegated-admin enforcement design (a separate document referenced by ADR-0010).
...
* Imported into this repository and translated in 2026-07, then reconciled against the design corpus on 2026-07-25 to restore the ABP migrator issue number, the parallel-change and ring-rollout attributions, the research-record pointer, and the F10 and F48 finding labels that carry this ADR's own traceability. Framework and library citations (EF Core, Finbuckle, the ABP migrator, and the parallel-change pattern) are named as neutral technical precedent.
