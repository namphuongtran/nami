...
* **`may_act` is de-scoped, and this one is a security decision rather than a scope one** (added 2026-07-26). RFC 8693 places `act` in section 4.1 as the carrier that records *that* delegation occurred and identifies the acting party. Section 4.4 defines `may_act` as "a statement that one party is authorized to become the actor and act on behalf of another party", and describes its use permissively: "The claim **might** be used, for example, when a `subject_token` is presented to the token endpoint ... and `may_act` claim in the subject token **can** be used by the authorization server to determine whether the client ... is authorized to engage in the requested delegation or impersonation." The section contains **no RFC 2119 requirement keyword at all**, so nothing in the specification requires an authorization server to issue or consult it. What the specification also does not do is state that other means are permitted, so this decision does not rest on a permission the spec grants; it rests on the spec not requiring the claim, and on Nami's model answering the same question better. Nami emits `act` and **never emits, stores, or validates `may_act`**. Its answer is the live, time-bound, revocable server-side grant checked on the resolved initiator (ADR-0047, ADR-0010): baking delegation authority into a token would make that authority **stale and un-revocable**, which is precisely the property the authorization model exists to avoid, and it would put the confused-deputy defence inside a bearer artifact instead of behind a check. This is therefore not a "no use case yet" de-scope like the ones above and would not be reopened by demand; reopening it would require the authorization model to change first.
...
* Related decisions: ADR-0002 (external IdP integration; dynamic per-tenant federation deferred), ADR-0003 (server-side session store, the basis for interim back-channel logout), ADR-0004 (refresh grant), ADR-0009 (`private_key_jwt` for M2M), ADR-0019 (single logout strategy, which supersedes the back-channel wait), ADR-0034 (dynamic per-tenant external IdP, the additive v2 feature), ADR-0056 (the FAPI 2.0 deferral that would build the de-scoped message-signing tier), ADR-0069 (proposed, the verifiable-credential issuer extension, another advanced-protocol scoping decision), and ADR-0070 (which reuses this terminate-and-forward model for local-development TLS). The DPoP mini-spec is a separate design document.
