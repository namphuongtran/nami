...
| csp-no-relaxation | the UI response profile's `script-src` and `style-src` carry none of `unsafe-inline`, `unsafe-eval`, `wasm-unsafe-eval`, or `unsafe-hashes`, and no `nonce-` source, in every environment including Development | enforces ADR-0091, **added 2026-08-01** |
| framing-denied | every browser-facing response profile sets `frame-ancestors 'none'` and `X-Frame-Options: DENY` | enforces ADR-0091, **added 2026-08-01** |
| response-headers-registered | the browser-facing response-header middleware is registered in the pipeline, in every environment | enforces ADR-0091 parameter K, **added 2026-08-02** |

The invariants split into two kinds. Some are the executable enforcement of a decision owned elsewhere (PKCE mandatory, no implicit, rolling refresh under ADR-0004, asymmetric-only signing under ADR-0005, the three transport rows under ADR-0076, the admin-scope row under ADR-0020, the permission-switch row under ADR-0001 and ADR-0035, and the three response-header rows under ADR-0091). The rest are hardening parameters that had no prior ADR home and are fixed by this ADR: S256-only PKCE, the JWE `A256CBC-HS512`/no-RSA1_5 pinning, the cookie-attribute set, and the no-degraded-mode-in-production guard. A test asserts that the self-check runs at startup and fails fast when any invariant is violated.
...
**The two response-header rows added 2026-08-01, and one thing they deliberately do not assert.** ADR-0091 fixes the browser-facing response headers as three profiles, and these two rows guard the parts of it that a consumer can weaken through ordinary configuration (ADR-0031), which is the same reason ADR-0076's transport parameters are enforced here rather than left as defaults. `csp-no-relaxation` extends to **Development** on purpose, because ADR-0091 parameter G establishes that the local hot-reload path needs only a wider `connect-src` and no script relaxation at all, so an `unsafe-inline` appearing in any environment is drift rather than a local convenience. What these rows cannot cover is the `form_post` response: it carries its own profile admitting one script by hash, and a startup check cannot issue an authorization request to see that response. That obligation is a contract test over a version-sensitive engine template instead, tracked as an ADR-0021 seam, and the split is stated here so the absence of a `form_post` row is not read as an omission. **This paragraph said "the absence of a third row" until 2026-08-02**, when a third row arrived for an unrelated reason; the sentence is corrected rather than deleted, because what it was defending is the `form_post` hash and that is still true.

**`response-headers-registered` added 2026-08-02, and it is the only row here asserting *reachability alone*.** Every other row reads a value, an option, a key, a cookie attribute, a directive, and `hsts-enabled-outside-dev` reads both, since it asserts the middleware is registered **and** that `max-age` clears the product default. This one has no value to read: it asks whether the middleware that would apply any policy is in the pipeline at all. ADR-0091 parameter K is why: that ADR fixes three response profiles, and until parameter K nothing said which responses receive one, so a consumer on the ADR-0027 parameter G path could serve their own login page with no policy while `csp-no-relaxation` and `framing-denied` both passed, since those two assert what a profile *contains* and a page outside every profile contains nothing to check. **The precedent for the shape is already in the table**: `hsts-enabled-outside-dev` asserts that "the HSTS middleware is registered", and ADR-0091 parameter A says it follows ADR-0076 parameter A exactly, so the enforcement follows too. What this row deliberately cannot see is a single response: registration is a property of the pipeline and a startup check has no response in hand, so the default-profile half of parameter K is covered by a test in ADR-0091's Confirmation rather than here.
...
* A test asserts each of ADR-0091's three response profiles on a representative response, that no response carries a `nonce-` source, and that startup fails when any of the three header rows is violated, including in Development. For `response-headers-registered` the violation to build is a host composed without the middleware, which is the consumer-written-host case ADR-0091 parameter A reasons about, not a configuration value.
