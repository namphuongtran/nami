...
* **Anti-bypass invariants** (the Intune lesson): dual-control is enforced in the Application layer (the `Application/` folder in `Admin.Api`, not the controller or UI), so no path executes a destructive action without going through the proposal saga; and the Admin API accepts no app-only token, no client-credentials client is registered with an admin-api scope, and the API policy `RequireActor` rejects any token lacking a `sub` (a real user). The break-glass path (ADR-0015) is separate and does not go through the Admin API.
