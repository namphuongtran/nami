...
* Revisit trigger: stays `proposed` until a customer needs SAML 2.0 or WS-Federation. At that point, confirm the current SAML 2.0 / WS-Federation interop expectations, decide build-versus a permissive .NET library (ADR-0026), reconfirm the account-linking and claim-allow-list invariants (ADR-0002), and run a security spike for the XML-signature attack surface (signature wrapping, XXE), before accepting or building.
