...
* **Client secret.** `OpenIddictApplicationDescriptor.cs` documents at its `ClientSecret` property (source-read at the 7.5.0 tag) that the value "may be hashed or encrypted for security reasons" **depending on the application manager used**, and the default `IOpenIddictApplicationManager.CreateAsync` hashes it. Two rules follow, and the second is the load-bearing one: Nami generates a random secret, passes the plaintext into `CreateAsync`, shows the plaintext **exactly once** to the tenant-admin, stores no plaintext, and does **not** use the secret store; and because the stored form is **manager-dependent by contract**, no Nami code may ever depend on reading a client secret back out. A rotation is therefore a re-generate-and-show-once, never a retrieve. The same descriptor also records that shared-secret client authentication is not recommended and exists mainly for legacy clients, which reinforces steering self-service clients toward a key pair or mTLS (ADR-0009) rather than treating a secret as the default. This is deliberately distinct from ADR-0034: an external-IdP secret uses the secret store because there the IdP-core *is* the client, whereas here the client secret is verified *by* the IdP-core, so OpenIddict hashes it. The two must not be conflated.
...
* Source-verified at the OpenIddict 7.5.0 tag: the application descriptor's `ClientSecret` documentation states the value may be hashed or encrypted depending on the manager, and the default manager hashes it on `CreateAsync`. Hence show-once, store-no-plaintext, no secret store, and never read a secret back.
* The requestable-scope rule rests on the global-scope-catalogue-plus-client-grant-allowlist finding (research record R18), which is also where the `TenantScopeEntitlement` authority comes from.
