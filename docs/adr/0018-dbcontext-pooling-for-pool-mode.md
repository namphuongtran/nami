...
* **Per-context matrix**: the global contexts (Identity, Data Protection, control plane) are pooled (a fixed connection, safe); **Silo** contexts are **not** pooled (their connection string varies, which is incompatible with `AddDbContextPool`); the **Pool-mode OpenIddict** context is **not** pooled in v1 either (this decision), because T7 showed a pooled instance carries a stale `TenantId` into the next request.
