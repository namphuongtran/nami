...
* This engine and its `ICheckAccess` port are the canonical authorization seam of the project, recorded in the **design corpus's** authorization, membership and delegated-admin design (its foundational "DB-first, ReBAC-ready" principle, its ReBAC model and swappable-abstraction sections, and its item 18). Those numbers are the corpus's; the owner here is [07](../design/07-authorization.md). The v1 `DbCheckAccess` SLO and the check timeout are interim-accepted and await Ops/Security ratification.
