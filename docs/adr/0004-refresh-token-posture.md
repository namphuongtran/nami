...
* **Absolute refresh lifetime ceiling**: rolling refresh gives sliding lifetime only, so a hard ceiling is stamped as an anchor on `Authorization.Properties` and enforced in the `HandleTokenRequest` handler, which rejects the request once the absolute expiry has passed.
