...
**That manifest exists as of 2026-08-02**, created with the first package reference, so the durable fix moves from blocked to open. It is not yet worth automating against one row, and the row it holds needs no entry here: `Microsoft.CodeAnalysis.PublicApiAnalyzers` is the mechanism of ADR-0044 parameter A, and ADR-0044 is already an owner of the "Packaging and distribution" row above. What is worth stating is the direction the check has to run. Reconciling this table *to* the manifest would find nothing for a long time, because a manifest of build-time analyzers is not a stack; the omission this blind spot describes is a technology named in ADRs and designs with no row, and only the manifest side of it is now mechanical. Wire the check when the manifest carries runtime packages, and until then the human step above is still the whole of it.
