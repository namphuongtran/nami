...
* **B. Account-lockout DoS mitigation.** Failures are scoped **per source IP alongside** per-account lockout, so fail-spam from one IP does not fill a victim's lockout counter. A distinct "many lockouts on one account" alert is raised, separate from the brute-force-spike alert, so the two attack shapes are told apart. **Which lane produces that alert, and the evaluator that produces it, are ADR-0082 and ADR-0083.** When this ADR was written neither existed: a per-account alert cannot be a metric under ADR-0077, and the security event carried no groupable subject key, so the alert this bullet promises had no mechanism behind it for as long as it stood alone. The **break-glass account (ADR-0015) is exempt from lockout** so an attacker cannot lock the rescue path; it is instead protected by challenge, alerting, and audit, and admin-unlock for ordinary accounts already exists in the admin surface.
...
* This posture was added in the 2026-07-13 pre-implementation review, in the **design corpus's** security-and-key-management document (its abuse-and-bot-defense section, findings L2-1/L2-2), with the device-flow and PAR DoS controls from that corpus's advanced-flows-and-standards document (its tasks 7.3a-d and 7.7a, the latter from the 2026-07-05 review). Those document and task numbers are the corpus's: in this repository the same subjects are owned by [11](../design/11-login-consent-ui.md) and [14](../design/14-advanced-flows.md), and the digits do not line up. Password-breach checking and the credential-hardening baseline are a separate, adjacent decision (part of user management) and are not covered here.
