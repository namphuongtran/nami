...
* **Gating (so it is not a backdoor)**: a feature flag `EmergencyAccess:Enabled` defaulting OFF, an IP allow-list (returning 404 to hide the endpoint), binding to an internal admin-network listener, a policy carrying `AuthenticationSchemes.Add("BreakGlass")` plus a role, and least-privilege repair-only, time-boxed access.
* **Sealed credential**: two break-glass accounts (not ordinary user rows), whose hashes are verified with `PasswordHasher<T>` from the secret store; **dual-control unseal** (a password and a second factor held by two different custodians); rotated after every use; never expired by disuse. Microsoft Entra recommends FIDO2 / certificate-based, phishing-resistant authentication for emergency accounts, which is stronger than a password hash, so the password hash is the baseline and an upgrade to FIDO2/CBA is to be considered at ratification. Break-glass hashes use the **hardened** iteration count of ADR-0028 section E, not the framework default: these are the most privileged credentials in the deployment, so the one place that must not silently sit at the default is this one.
...
* **Turnkey first-admin for the reference image**: for the zero-code reference host (ADR-0027), a `Bootstrap__Admin*` environment configuration (email/password, or a client id/secret for automation) is applied **once** at first start when no admin exists. It creates the user **and the root delegated-admin grant** (ADR-0010, so the account has real authority rather than only a login), forces a password change, audits `admin.bootstrap`, and fails fast in Production on a weak or absent value. Apply-once plus forced-change means this is not a standing default credential (the risk the setup-token path above also avoids); it is the container-friendly variant of first-admin seeding, matching the turnkey bootstrap flags of Keycloak and Zitadel. The account stays **temporary by intent**: the operator is expected to create a durable admin and then disable the bootstrap path.
...
* Evidence records behind this decision: a survey of break-glass and first-admin patterns (Entra, AWS SEC03-BP03, Keycloak, NIST SP 800-53 AC-2) and a separate record of the .NET emergency local-authentication mechanics. The gap was first raised in the admin-portal phase document with no ADR attached to it, which is how it stayed a hidden SEV-2 risk until this ADR.
