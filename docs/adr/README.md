...
| [0086](0086-pin-ci-actions-by-version-tag.md) | Pin every CI action by a full version tag rather than a floating major or a commit SHA, kept at the latest stable release and enforced by the guardrail; parameter A reversed 2026-08-02 | accepted |
...
| [0088](0088-claims-contract-stability.md) | Freeze the claims contract as a published consumer surface from v1.0, splitting the promise by who owns each name so Nami commits to the five claims it defines and not to the syntax of the five that OIDC and RFC 8176 define | accepted |
| [0089](0089-self-service-surface-conventions.md) | Give the self-service surface its own conventions while it has one endpoint: identity comes from the token so no path carries a subject, and lowercase kebab-case replaces the camelCase route names inherited from the framework surface this design rejects | accepted |
| [0090](0090-versioned-api-base-path.md) | Serve Nami's own APIs under the base path /api/v1 with route templates staying relative, and decide by a three-clause test rather than per endpoint which of the eight route families carry a version, so a probe path and a provider webhook keep theirs | accepted |
| [0091](0091-browser-facing-response-headers.md) | Fix the browser-facing response headers as three profiles chosen by response class, deny framing outright because ADR-0014 and ADR-0019 already removed every iframe Nami had, and admit by hash the one inline script the engine writes for response_mode=form_post, which a strict script-src would otherwise break as a blank page with a clean server log | accepted |
| [0092](0092-ci-security-scan-tooling.md) | Pin one tool to each of the five CI security scans and take no third-party SAST engine, because the pinned .NET SDK already ships the taint-analysis rules that stage exists for, which leaves the two encumbered candidates unnecessary: Semgrep is kept as the named reversal candidate and the CodeQL CLI is rejected for binding the gate to a public repository on one host rather than for its licence alone | accepted |
