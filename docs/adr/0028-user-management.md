...
* **Amendment, 2026-08-01: the passkey wiring this ADR calls B(c) must carry user verification, and that is a security property rather than a detail of the wiring.** B(c) treats a passkey as a primary factor combined with step-up and AAL (ADR-0013). What was missing is the condition under which a passkey counts as **two** factors: without user verification the assertion proves possession of the credential and nothing else, so the assurance design's "every passkey is aal2" granted a tier a single-factor login had not earned. `IdentityPasskeyOptions.UserVerificationRequirement` (applies to both ceremonies, default `"required"`) and `UserPasskeyInfo.IsUserVerified` are both native on .NET 10, verified at the 10.0.9 reference assemblies, so this costs no new column, no new dependency, and no migration. ADR-0013 carries the assurance rule; the user-management design section 5.2 carries the mechanics and names what is not verified.
* Build-time follow-ups: passkey endpoint mapping, requiring user verification in both ceremonies **and asserting the resulting flag rather than the request parameter**, the attestation-validation policy, and the UI; the profile schema and lifecycle state machine (invite/approval/offboard) with audit provenance; the admin-console extension; and, from Security/DPO, the passkey attestation policy and PII profile retention.
