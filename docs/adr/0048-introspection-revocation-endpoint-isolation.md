...
* **Amendment, 2026-08-01: option C needs no build, and finding that out is a second instance of this ADR's own option B.** Decision C above is unchanged as a *requirement*, but the surrounding designs had read it as work to do. Read at source: `OpenIddictServerHandlers.Introspection.cs:733-742` assigns the confirmation by reading `Claims.Confirmation` off the token principal and parsing **the whole JSON object** through, with no branch on the binding method and no filter on the key name; `:239-241` writes it to the response; `:838` deliberately excludes `Confirmation` from the application-claims merge. So once issuance stamps `cnf` (which *is* Nami's build, ADR-0014), `cnf.jkt` is surfaced natively. What remains Nami's is only the **enrich-or-inactive policy**, which is a policy and not a mechanism. The origin of the error is named so it is not re-derived: verification record V14 concluded the engine's `cnf`-in-introspection covers only the certificate form, which is true of **issuance** (`CreateConfirmationClaim` stamps only `x5t#S256`) and not of introspection. This ADR already rejects reimplementing a native capability in application code as "the most common wrong-API mistake here"; scheduling a build for one is the same mistake one step earlier.
* **Also read at `:762`, and it changes the wire contract:** the engine emits `token_type: "Bearer"` only when the confirmation is **absent**, and its own comment cites RFC 7662 section 2.2 to explain that omitting the node beats claiming `Bearer` for a token that is not a bearer token. A response for a bound token therefore carries **no** `token_type`, so a resource server that requires the node breaks on exactly the tokens this ADR exists to protect.
