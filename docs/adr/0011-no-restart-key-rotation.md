...
* Evidence: openiddict-core issue #386 (no native signing-key store or automatic rotation) and issue #1434 (the maintainer recommending a custom `IOptionsMonitor`). The 90/14/14 rotation-interval / propagation / retention shape follows a common industry pattern also seen in mature commercial identity servers.
