...
* Bad, because it relies on a seam that the OpenIddict maintainer endorses (issue #1434) but that is not in the official OpenIddict documentation, so it is fragile across OpenIddict minor upgrades; this mandates an "options contract regression test" on every bump (7.5 → 7.6 → 8.0) that fails the build if the contract breaks.
...
* OpenIddict source verified: `AttachSecurityCredentials` reads `context.Options.SigningCredentials.First()`; `AttachSigningKeys` iterates the whole list without filtering on `NotBefore`; `UseLocalServer` snapshots into a static configuration manager plus change-token; issue #1434 records the maintainer recommending a custom `IOptionsMonitor`. **That sentence reports the issue, not this decision**, and the two differ deliberately: Nami satisfies the recommendation with the framework monitor plus a custom `IConfigureOptions` and change-token source, for the reasons in the fixed parameters above. Both wordings are kept so a reader who opens #1434 can see why the shapes do not match.
* Mandatory integration tests before relying on the seam, listed in the [testing design](../design/20-testing.md) section 5.7: tripping the change-token makes `UseLocalServer` self-validate a token signed with the new key without a restart; reading `CurrentValue` multiple times does not create a new key; the contract regression test runs on every OpenIddict version bump.
