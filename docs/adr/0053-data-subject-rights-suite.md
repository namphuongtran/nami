...
Mechanisms A-G are jurisdiction-agnostic. The jurisdiction profile (the supervisory authority, the breach deadline and target, the sensitive-data categories, the consent form, and the response SLAs) is a DPO/Legal-ratified input, with GDPR and Vietnam's Personal Data Protection Law (Law 91/2025/QH15, effective 1 January 2026) as the first two profiles. Vietnam differs in ways the profile must carry: the breach authority is the Ministry of Public Security (department A05) with a 72-hour deadline that, for sensitive, financial, biometric, or location data, also requires notifying affected subjects within 72 hours; and its sensitive-data categories add geographic location and credit-institution customer data. Cross-border transfer and data residency are handled in ADR-0054.
