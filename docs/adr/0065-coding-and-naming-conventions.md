...
**Both items moved on 2026-08-02, later the same day, when `Nami.Identity.Abstractions` became the first project.** The rules now read real C#, and both gates run against the solution in CI. Proven by breaking rather than by the green: a `badlyNamed` private field planted in the one type that project contains fails `dotnet build` with exit 1 and `dotnet format --verify-no-changes` with exit 2, both on IDE1006. **Neither item is closed, and the distance left is not the same for the two.** The first is barely started, because one type of three properties says almost nothing about how much real code the ruleset would reject, which is the whole content of that item; it closes gradually as `src/` fills and it is the one that will generate pressure to weaken a rule. The second is closed apart from the ArchUnitNET suite, which needs a test project and real types to reason about.
