...
A `View` selects the instrument it configures by name. **A view whose selector does not match the emitted instrument name configures nothing, and this repository has not been able to confirm from the OpenTelemetry .NET documentation that a non-matching view produces any diagnostic.** Verified on 2026-07-26: the documentation covers the view API and the cardinality limit, and states that `AddMeter` supports wildcard subscription, but says nothing about what happens to a view that matches no instrument.

So the mechanism is not trusted on documentation. A test asserts that the configured views are attached to the instruments they name and that the resulting limit is in force. This is the whole reason rule D exists as a separate binding parameter rather than an implementation note: **an unattached cap is indistinguishable from an attached one by reading the configuration**, and it reads as protection while providing none. The same reasoning is why ADR-0043 asserts its invariants at startup rather than documenting them.
...
* Bad, because rule D compensates for an undocumented behaviour, so the test is asserting something the SDK may change; accepted, since a test that becomes redundant is a better failure than a cap that was never live.
...
* **External verification, 2026-07-26, OpenTelemetry .NET.** The SDK's default cardinality limit is 2000 points per metric; a per-instrument limit is set through `MetricStreamConfiguration.CardinalityLimit` on the view API; and since 1.10.0 measurements past the limit are aggregated into a synthetic point carrying `otel.metric.overflow=true` rather than being dropped without trace. **Not verified:** whether a view whose instrument-name selector matches nothing emits any diagnostic. The documentation is silent, which is what rule D is a response to.
