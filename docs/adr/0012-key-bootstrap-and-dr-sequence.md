...
* Verify-before-build tests, both listed in the [testing design](../design/20-testing.md) section 5.7: auto-seed produces exactly one key across multiple nodes and readiness moves from fail to pass; and DR restore-both works, carrying the negative case proving that a missing keyring is detected rather than silently regenerated.
...
* Imported into this repository and translated in 2026-07, then reconciled against the design corpus on 2026-07-25 to restore two test obligations that the import had dropped: multi-node auto-seed with the readiness transition, and DR restore-both with its missing-keyring negative case. The corpus's own labels for them were carried here until 2026-08-01 and are now gone, because they resolved to nothing in this repository. References to a specific commercial identity server's key management stay generalized, including its initialization-window and synchronization-delay values; ASP.NET Core Data Protection framework references are named.
