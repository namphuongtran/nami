...
* **Spike-first, and the gate has already been cleared:** spike A-8 **ran and passed on 2026-07-10, 8 of 8** (verification record V28). It proved that a scheme resolves at runtime from the store rather than at boot, that a no-restart rebuild works (with both halves of the versioned cache proven: a cache hit within a version and a genuine rebuild after a version bump, so an always-rebuild no-op cache would have failed), that two tenants are isolated in scheme enumeration while the static scheme remains visible, and that a callback reaches `GetExternalLoginInfoAsync` with the correct provider and subject. Findings F-A8-1 through F-A8-4 are folded into the implementation design, F-A8-4 having been surfaced only by an adversarial review that tightened three of the tests.
...
* Bad, because the threat surface grows with a user-input Authority, so the SSRF and anti-takeover defenses must be tight; the gate spike ran first and passed, so the mechanism itself is no longer the open risk.
...
* v2-build follow-ups (the A-8 gate is already cleared, see above): Security ratifies the self-service threat model (whether an operator host allow/deny list is needed for the SSRF guardrail) and the DPO is consulted if an external IdP handles sensitive PII; and it is confirmed whether the self-service secret path needs operator dual-control (currently a per-tenant secret is self-service with audit and the port, and is not treated as a global IAM change).
...
* **Dynamic scheme provider (chosen)**: good, because it is no-restart, lets the framework handler own the protocol, and has been proven end to end by spike A-8; bad, because it is real machinery.
...
* Original decision 2026-07-09 (v2 scope-opened); the gate spike ran and passed the following day, 2026-07-10, so this ADR is no longer gated on unrun work. This re-opens the dynamic per-tenant IdP that ADR-0002 deferred, inheriting the ADR-0002 handler-based model rather than overturning it, and it is the additive v2 feature referenced by ADR-0014.
