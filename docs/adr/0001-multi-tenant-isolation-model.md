...
* This layering is realized as **five DbContexts**: `OpenIddictDbContext` (tenant-scoped: pool filter or silo connection), `IdentityDbContext` (global), `DataProtectionDbContext` (global), `ControlPlaneDbContext` (global: tenants, memberships, delegated admin, audit log, server-side sessions), and `ControlPlaneTenantDbContext` (tenant-scoped: the control-plane tables that are `.IsMultiTenant()` and RLS-isolated). This topology is fixed; changing it requires a superseding ADR.
* **The tenant discriminator column is `varchar(64)` text, holding `Tenants.Identifier`, and this is a deliberate exception to the surrounding uuid convention.** It is a clarification rather than a contradiction: the UUIDv7 rule (ADR-0036) governs **primary keys**, and a discriminator is not a primary key. The reason is mechanical, not stylistic. Finbuckle's tenant identity is a string (`ITenantInfo.Id` and `.Identifier` are both `String`), so `.IsMultiTenant()` and the auto-stamp, query filter, and throw-on-mismatch that come with it compose **only** against a text column; a `Guid` tenant property throws at model build and the application does not start. Probe-verified 2026-08-01 against `Finbuckle.MultiTenant.EntityFrameworkCore` 10.1.2. It follows that `ITenantInfo.Id` maps to `Tenants.Identifier` and not to `Tenants.TenantId`, which is what makes one representation carry end to end from the discriminator column to the `tenant` claim to the change-event envelope. `Tenants` therefore has two candidate keys in use on purpose: `TenantId` (`uuid`) as the foreign key for global tables referencing a tenant as data, and `Identifier` (text) as the discriminator. This is safe only because `Identifier` is immutable post-provision; relaxing that invariant means revisiting this. Detail in design [02](../design/02-data.md) sections 1 and 4.
