...
* **Tooling.** Static analysis and dependency scanning run in CI alongside the license-scan gate (ADR-0026) and the coordinated-disclosure/CVE process (ADR-0045); dependency scanning is already scaffolded (`.github/dependabot.yml`, NuGet ecosystem enabled at M1). **The analyzers were left an open, replaceable choice here and are now pinned by ADR-0092** (amended 2026-08-02). That ADR fixes one tool per stage and takes no third-party SAST engine, having found that the capability the stage existed for already ships in the pinned .NET SDK. This clause keeps stating the requirement, which is what an ASVS baseline owns; what runs it is no longer open, and the openness is what changed rather than anything in the requirement.
