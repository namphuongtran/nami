...
* Revisit trigger: stays `proposed` until an on-premises customer needs Windows integrated authentication. At that point, confirm the ASP.NET Core Negotiate handler behavior on the then-current .NET, reconfirm the account-provisioning and linking reuse (ADR-0002), and settle the Kerberos/SPNEGO deployment constraints (SPN, keytab, and behavior behind a TLS-terminating proxy), before accepting or building.
