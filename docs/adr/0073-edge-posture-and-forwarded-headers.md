...
* **Deliberately not decided here.** The application's own transport-security settings (the HSTS parameters and the TLS floor Kestrel enforces when it terminates TLS itself) are not currently fixed by any ADR: ADR-0043's invariants cover PKCE, signing, JWE, cookies, and degraded mode, but not transport. This ADR does not silently adopt them; it records the gap so it is a known open item rather than an assumed one.
