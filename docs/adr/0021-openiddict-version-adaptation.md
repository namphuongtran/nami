...
2. **Undocumented but maintainer-endorsed seams**: high risk, and they can break silently on a bump. Examples: the custom `IOptionsMonitor<OpenIddictServerOptions>` for no-restart rotation (issue #1434, ADR-0011, verification record V06); inserting event handlers by `SetOrder` around the built-ins for DPoP (spikes A-1 and A-3); the internal `ValidateProofOfPossession` throwing `ID2196` (spike A-3); and the Finbuckle-times-OpenIddict `OnModelCreating`/`SaveChanges` composition (spike A-4).
