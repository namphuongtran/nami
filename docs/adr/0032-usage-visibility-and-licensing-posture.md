...
* **The configuration identifiers here deliberately differ from the source corpus** and must not be "restored" to it. The corpus used a `NAMI_IDENTITY_TELEMETRY` environment variable and a `nami-identity-registration.key` filename; this repository uses `Nami:Telemetry:Enabled` (environment `Nami__Telemetry__Enabled`) with the short alias `NAMI_TELEMETRY`, and `nami.registration.key`, because ADR-0065 is the naming authority and fixes exactly that shape (`Nami:X` in configuration, `Nami__X` in the environment, and a short `NAMI_X` alias for common toggles). The corpus predates that consolidation.
