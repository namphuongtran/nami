...
* **Opportunistic immediate dispatch, promoted from an option to the decision on 2026-08-01.** The outbox bounds *worst-case* delivery but makes the *common* case wait for the next relay claim. A best-effort dispatch is attempted right after the response is written, **with the outbox row already committed in the same transaction as the session revoke**, so the common case is near-immediate while the guarantee still comes entirely from the outbox: a failed or skipped attempt changes nothing, because the row is still pending. This is the same shape the email design already uses for its most critical message, a priority lane described there as sync-with-fallback (ADR-0038). Two constraints bound it, and they are now requirements rather than caveats: it must **not** block the interactive logout on N relying-party calls, which is the latency the outbox exists to remove (ADR-0003), and it must not become a second delivery path with its own retry semantics, because then a relying party could receive two logout tokens governed by two different rules. The dispatch therefore performs **at most one attempt per relying party and never retries**; every retry belongs to the relay. The mechanism is in design [11](../design/11-login-consent-ui.md).
* **Why that promotion, and why the alternative was rejected (F13, 2026-08-01).** The question was raised by the design corpus, whose review finding H-12 replaced its own outbox with inline-first delivery. Checking it against this repository changed the answer, on three findings.
  * **The corpus's own decisive reason does not apply here.** Its detailed design states two reasons and says "the second is why the shape changed": that a queue-first design *forced* the delivery bookkeeping to be tenant-scoped, which is incoherent with a global session. That defect is real, and this repository already fixed it on 2026-08-01 by making the delivery table class B **global** with no row-level security, which is a smaller change than removing the outbox. What remains of the corpus's case is its first reason, cost and latency, and the dispatch above answers the latency half.
  * **The corpus's remedy would require retracting an accepted decision here.** Its mitigation for the residual is to "enqueue failures **before** completing the logout response", which means the response waits for the parallel POSTs or their timeout, stated there as of the order of two seconds. ADR-0003 and design 11 both require that the fan-out **never blocks interactive logout**. Committing the row first and dispatching after the response keeps that rule and needs no residual, because nothing is lost if the dispatch never happens.
  * **A fire-and-forget variant was considered and is worse than either.** Dispatching without persisting first, and writing only failures, cannot use the corpus's ordering mitigation at all, so its loss window is wider than the corpus's rather than narrower.
  * **On cost:** the corpus's spike A-9 (verification V29) proves the transactional-outbox chassis under concurrency, draining 6,400 events at roughly 7,072 per second with zero loss. It is worth being precise about what that does and does not support: A-9 covers the **v2 identity-change-event** feature and contains no reference to logout at all, and its own record states it proves correctness under concurrency rather than production sizing. It is evidence that this pattern is sound and cheap, not evidence about logout specifically.
