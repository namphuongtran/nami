...
* **`LangVersion` is deliberately not in that file, which is a deviation from parameter C and is recorded rather than quietly taken.** Parameter C names it as one of the things the knob centralizes. Measured on SDK 10.0.301 with no `Directory.Build.props` present at all, a `net10.0` project already reports `LangVersion 14.0`, because the default is derived from the target framework: the knob therefore already centralizes the language version, through the framework, and the two stay in step by construction. Writing the property could only restate that default, which rots against the SDK, or set `latest`, which would float the language version with whatever SDK is installed and **break** the derivation parameter C wants. This is ADR-0065's "the ruleset writes only what deviates" rule applied to a second file. Revisit if a future bump ever needs a language version that is not the framework's default.
* Build-time follow-ups: ~~create `Directory.Build.props` (the target-framework knob), `global.json` roll-forward,~~ and the CI matrix (a single-LTS host and multi-target packages) plus the early-warning job on the next preview, folded into the shared contract-regression suite that runs on every .NET and OpenIddict bump; and the quarterly Ops roadmap-watch of EOL/GA dates. Both struck items closed on 2026-08-02. What is left of parameter C is keeping the Dockerfile SDK/runtime pin in step, which needs a Dockerfile, and MinVer with the reproducible-build stack, which needs something to version.
