...
* **I. Configuration keys and shipped static-asset paths are part of the surface too (added 2026-08-01).** An operator's configuration file is as downstream of Nami as a compiler is: renaming `Nami:Email:Provider` breaks every deployment that upgrades, and moving a shipped static-asset path breaks every consumer who replaced a file at it. Both are therefore governed by the rules above. A rename or a move is MAJOR under B; adding a key with a default is MINOR under B's "new option with a default"; and a removal takes C's two-step window, expressed for configuration as the old key continuing to bind for at least one minor while startup logs a warning carrying a `NAMIxxxx` diagnostic id. **This is parameter G's rule with a different surface**, since G already accepts that a consumer-facing name no analyzer can see is still contract, on the ground that renaming a metric breaks dashboards. Parameter A's forcing function genuinely does not reach here, because `PublicAPI.Shipped.txt` holds types and members rather than `Nami:` keys or file paths, so the equivalent is a checked-in manifest of the keys and asset paths each package owns, and a change to that manifest is reviewed exactly as an API-file change under H. What this closes is an asymmetry inside ADR-0065's own list: its protocol-URN bullet says those strings are "a stable public contract under ADR-0044 and cannot be changed without a version bump", while its configuration-key bullet said only what a key looks like.
...
* Bad, because maintaining the `Shipped`/`Unshipped` files, the deprecation windows, the port-evolution discipline, and (from 2026-08-01) parameter I's manifest of configuration keys and asset paths is real ongoing overhead on every change. The manifest is the weakest link of the set, because it is the only one no analyzer maintains, so it decays exactly when a change is rushed.
...
* CI fails a PR that renames a configuration key or moves a shipped static-asset path without updating the manifest parameter I requires, which is the same gate H applies to the API file.
