...
* The spike surfaced **three findings that changed the design rather than confirming it**, which is why it ran first:
  1. A row-level-security policy comparing a **uuid** tenant column must use `NULLIF(current_setting(...), '')::uuid`, because a **pooled** connection returns an empty string rather than NULL after the transaction ends, and casting an empty string to uuid **throws** instead of failing closed. The scope of the trap is the tenant column's **type**, not the release: it does not reach the OpenIddict tenant tables, whose discriminator is the `text` Finbuckle identifier, where an unset variable simply fails to match and therefore fails closed. It does reach every row-level-security-guarded table with an explicit `uuid` tenant column, and **v1 already has four of those**: `LogoutDeliveryOutbox`, `OutboxEmail`, `SuppressionEntry`, and `ProcessingRestriction`. All four are handled in the data design and in the designs that own them, which specify the `NULLIF` predicate form and the `22P02` cast failure it prevents; this ADR's v2 outbox follows that existing rule rather than introducing it. A fifth v1 table, `TenantBranding`, carries a `uuid` tenant column as its primary key but has **no row-level-security policy recorded anywhere in the design**, which is an open item rather than a decided exemption and is tracked as such. (An earlier draft of this entry said "v1 is unaffected, since its tenant column is text", which was corrected on 2026-07-25: it was true of the OpenIddict discriminator and wrong about the control plane, and it contradicted the data design that had the rule right.)
