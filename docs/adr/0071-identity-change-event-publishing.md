...
* The spike surfaced **three findings that changed the design rather than confirming it**, which is why it ran first:
  1. A row-level-security policy comparing a **uuid** tenant column must use `NULLIF(current_setting(...), '')::uuid`, because a **pooled** connection returns an empty string rather than NULL after the transaction ends, and casting an empty string to uuid **throws** instead of failing closed. The scope of the trap is the tenant column's **type**, not the release: it does not reach the OpenIddict tenant tables, whose discriminator is the `text` Finbuckle identifier, where an unset variable simply fails to match and therefore fails closed. It does reach every row-level-security-guarded table with an explicit `uuid` tenant column, and **v1 already has five of those**: `LogoutDeliveryOutbox`, `OutboxEmail`, `SuppressionEntry`, `TenantBranding`, and `ProcessingRestriction`. All five are handled in the data design, which carries the authoritative list and specifies the `NULLIF` predicate form and the `22P02` cast failure it prevents; this ADR's v2 outbox follows that existing rule rather than introducing it. (An earlier draft of this entry said "v1 is unaffected, since its tenant column is text", which was corrected on 2026-07-25: it was true of the OpenIddict discriminator and wrong about the control plane, and it contradicted the data design that had the rule right. A later draft then listed only four tables and flagged `TenantBranding` as an open item, because no design recorded a policy for it; that item was **closed on 2026-07-26** in favour of guarding it like every other per-tenant control-plane table, so the count here is five. The read-path consequence is recorded with the table in the data design: the login surface must resolve the tenant before reading branding, or the cast fails closed and the default theme is served.)
