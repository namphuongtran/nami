...
* **Deliberate exception, the BFF proxy** (final-review finding F40, 2026-07-05): `Nami.Identity.Bff` (composing YARP plus `Duende.AccessTokenManagement.OpenIdConnect`, see ADR-0029) is a real infrastructure edge but has no port. It is a deliberate composition boundary, not a port: the seam is in configuration (route/cluster/token-management options) and the adapter is YARP plus the token-management library itself. It does not meet the "at least two real reasons for a port" bar (no engine swap, and no need for an in-process fake since it is tested over HTTP), so the "a port must have a real reason" doctrine is honored by not wrapping the library in a port. This is an explicitly acknowledged exception to the ports-at-the-edge rule.
...
* Maxim (Jimmy Bogard, who originated the vertical-slice framing): minimize coupling between slices, maximize cohesion within a slice.
...
**Endpoint mechanism (orthogonal to slice organization):** the Admin API keeps REST controllers (per ADR-0020; controllers fit where advanced model-binding/validation is needed), and the IdP-core keeps controllers for the OpenIddict pass-through endpoints. That set is fixed by the engine rather than chosen here, and naming it loosely has caused a real error, so it is named exactly: authorize, token, userinfo, end-session, and **end-user verification**. The engine offers a pass-through option for those five and for its error page, and for nothing else, which means introspection, revocation, and the **device authorization** endpoint cannot take a controller at all. An earlier version of this sentence said "logout" for what the engine calls end-session, and "device" for what is really only the end-user verification half of the device flow; the core-protocol design carries the source-verified table. Minimal APIs are used for auxiliary endpoints where convenient, with no mass migration.
...
**Enforcement:** an architecture-test suite (`Nami.Identity.ArchitectureTests`) using TngTech.ArchUnitNET (Apache-2.0, actively maintained in 2026, verified on NuGet 2026-07-04), chosen over the original `NetArchTest.Rules` (unmaintained since 2021), with `NetArchTest.eNhancedEdition` (an MIT fork) as a drop-in alternative. These documents use the name ArchUnitNET consistently for this suite; the unmaintained `NetArchTest.Rules` is a different library and is not used. Rules: (a) Domain references no OpenIddict/EF/cloud SDK; (b) Application references no cloud SDK or engine; (c) a slice does not reference another slice (only via Contracts/Domain); (d) cloud adapters live in Infrastructure; (e) `Nami.Identity.Bff` must not reference `Nami.Identity.Admin.*` (the BFF package is shared by admin and SPA consumers, so admin routes/policies stay in `Nami.Identity.Admin.App` and no coupling leaks into the BFF layer; final-review finding F41, 2026-07-05). A slice template and a code-review checklist compensate for the guardrail that vertical slice removes.
...
* The 2025-2026 literature (Jimmy Bogard on vertical-slice architecture, Milan Jovanović on slices with shared logic in a modular monolith, and CodeOpinion on vertical-slice versus Clean) treats vertical slice and Clean as complementary: ports/adapters for technical coupling, vertical slice for cohesion, a module boundary plus the dependency rule at the macro level and slices at the micro level, with vertical slice fitting handler/CQRS and CRUD reducing to thin slices.
* Not independently verified: there is no explicit statement from OpenIddict's maintainer (Kévin Chalet) opposing Clean Architecture, only the implicit signal of the flat sample, and "framework-owned flow" is Nami's own phrasing, though the vertical-slice-to-handler/CQRS mapping is verified.

**The enforcement clause went live on 2026-08-02**, on the first of the five rules and the one project `src/` contained. `tests/Nami.Identity.ArchitectureTests` exists, uses TngTech.ArchUnitNET as this ADR requires, and runs in CI as its own job. It asserts design [01](../design/01-foundations.md) section 3.1's "`Abstractions` depends on nothing". Rules (a) through (e) above are otherwise untouched and remain unenforced, because the projects they constrain do not exist yet.

* **The suite is two facts for one rule, and that is not redundancy.** One reads the type graph through ArchUnitNET, the other the assembly reference table through reflection. Measured against a `Newtonsoft.Json` reference planted in `Nami.Identity.Abstractions`: with a type calling into it both fail; with the package referenced and no code touching it **both pass**, because an unused reference is elided from metadata. That third case is the suite's stated limit and it is written into the test file. What would close it is a check on the packed surface, which needs a pack and does not exist.
* **The formulation that reads correctly in English is the inert one.** `OnlyDependOnTypesThat().ResideInNamespaceMatching(...)` passed over the planted violation while `NotDependOnAnyTypesThat().DoNotResideInNamespaceMatching(...)` failed on the same architecture. The loader was given one assembly, so `Architecture.Types` holds only that assembly's types, and a rule that resolves its allowed set out of that collection can never find a foreign type to reject. The dependency is recorded regardless: dumping the offending type's `Dependencies` showed `Newtonsoft.Json.JsonConvert` present. **So a rule written in this suite is not enforcing anything until it has been failed on purpose**, which is the same lesson ADR-0044's Confirmation records about `RS0017` and ADR-0065's about `.editorconfig` severity, now reached a third time by a third mechanism.
* **The test must not assert on `PackageReference` items**, and the reason is measured rather than stylistic: `Nami.Identity.Abstractions` legitimately references an analyzer with `PrivateAssets="all"`, which reaches neither the type graph nor the reference table, and both facts are green with it in place. A test reading the project file would fail on a correct project and the obvious "fix" would be deleting the analyzer that ADR-0044 parameter A depends on.
...
* Related decisions: ADR-0020 (two admin projects, the Application folder, and ArchUnitNET), ADR-0001/0005/0006/0008/0009/0010 (the infrastructure ports), ADR-0021 (the seam catalogue), ADR-0022 (the two-lane logging), ADR-0029 (the BFF, the acknowledged port exception), ADR-0058 (the Separation-of-Concerns and pragmatic-SOLID principles this style applies), ADR-0060 (the testing strategy, including the ArchUnitNET architecture tests defined here), ADR-0065 (the coding and naming conventions that consolidate this ADR's namespace and slice-folder rules), and ADR-0066 (design patterns applied pragmatically, which reuses this ADR's start-simple and no-single-implementation-interface rules).
* Imported into this repository and translated in 2026-07; content preserved, internal references generalized. The commercial identity server, its documentation URLs, and its internal storage-abstraction type names stay generalized; the product-name placeholder was set to the repository's `Nami.Identity.*` convention. Three sets of names the first import had generalized are restored (2026-07-25), because no content rule reached them and this repository already cites authors elsewhere (ADR-0017 names Fowler): the OpenIddict maintainer in the negative claim above, the vertical-slice literature authors whose work this ADR relies on, and the BFF token-management package, which is now named per ADR-0026 section E rather than described as a "non-vendor-branded choice" this project never actually decided. Test libraries and the Microsoft reference material are retained as neutral technical references.
