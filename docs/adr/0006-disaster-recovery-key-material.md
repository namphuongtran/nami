...
* **Verify restorability automatically and often, between the drills** (added 2026-07-26). The two controls above leave a gap between them: continuous monitoring catches a backup that **stopped**, and the quarterly drill catches a backup that **cannot be restored**, so a backup that runs on schedule but produces an unrestorable artifact has a fresh age, raises no alert, and is discovered up to a **quarter** later. An automated probe closes that window by restoring the most recent backup into a throwaway target on a short cycle (weekly is the starting cadence) and asserting a cheap liveness property on the result, such as the schema version and a row count on a known table. It is deliberately **shallower than the drill**: it proves the artifact restores, not that the service recovers, and it does not replace the quarterly evidence-producing drill or its pass criterion. Its own failure is an alert of the same class as archiving lag, since a probe that has silently stopped running reproduces exactly the blindness it was added to remove.
