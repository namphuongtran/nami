...
* **D. Anti-enumeration.** `/forgot-password` and `/resend-confirmation-email` always return the same response with the same latency whether or not the account exists or is confirmed; the lookup runs and silently skips on failure with no HTTP or timing branch. Endpoint rate limiting keys on IP plus email-hash. A latency-invariance test is a mandatory acceptance invariant.
...
* **The two endpoint names in section D were re-spelled on 2026-08-01, and nothing this ADR decided changed.** They read `/forgotPassword` and `/resendConfirmationEmail`, which are `MapIdentityApi`'s own camelCase route names, while the user-management design rejects `MapIdentityApi` outright as "a parallel JSON attack surface". Naming them that way made the route list read as though that surface were mounted, which is a false signal in exactly the review that would look for it. [ADR-0089](0089-self-service-surface-conventions.md) settles the self-service surface's casing as lowercase kebab-case, matching every path segment on the admin surface, so they are now `/forgot-password` and `/resend-confirmation-email`. This ADR decides the **anti-enumeration behaviour** of those endpoints and never decided their names; the rename is ADR-0089 filling a gap rather than overturning a decision, and it is recorded here because a silent edit to an accepted ADR is worse than the drift it fixes.
