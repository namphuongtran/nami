...
### 3.1 The shared subject data-map
...
### 5.1 `ISubjectErasureService` (idempotent, resumable saga)
...
### 5.2 Audit crypto-shred and the per-subject key
...
2025-09-26). EDPB Guidelines **02/2025**, whose subject is blockchain, recognize that
destroying the decryption key can render data unintelligible.

Four things about that document are **not verified at the primary source** and must be read
there before any of them is relied on, because a mis-stated regulatory citation misleads the
reader it is written for. Its **publication status**: the design corpus records it as adopted
in April 2025, while an earlier draft of this text called it a public-consultation document,
and neither reading has been checked against the EDPB's own record. Whether the guidance is
**conditional** on the algorithm remaining unbroken and the key uncompromised. Whether the
phrase "encrypted personal data is still personal data" appears in **that** document or comes
from the separate EDPB pseudonymization guidance and WP216, which is where this design's own
pseudonymization framing (below) is drawn from. And whether the route it endorses is
anonymizing by deleting off-chain identifying data. Each is a claim about a legal text rather
than about this system, so it is DPO/Legal's to confirm at the source, and this design does
not assert it either way.

What does not depend on that verification: treating key destruction as Article 17 erasure
**in general** is the controller's reasoned position rather than a settled
data-protection-authority rule, and the residual ciphertext remains personal data until the
key is destroyed. Two further cautions stand on their own. The 2019 EDPB guidelines sometimes
cited in this context concern search-engine de-listing, not cryptographic erasure, so citing
them here would be a category error. And any secondary "a supervisory authority recognizes
crypto-shred" claim is unverified until read at that authority's own publication.

### 5.3 `ILegalHoldService` and `IRetentionPolicy`
...
### 5.4 Data-subject rights beyond erasure
...
### 5.6 Breach and record hooks (Art.33/34/30/35)
...
### 5.7 Cross-border transfer register and jurisdiction profile
