...
([12 Admin API](12-admin-api.md)); key escrow, destruction, and the DP-key
delete-is-irreversible caveat ([09 key management](09-key-management.md)); the
revoke-by-subject and force-logout primitives ([10 revocation](10-revocation-caching.md),
[08 UI](08-login-consent-ui.md)); the self-service profile-edit and change-email
mechanics ([06 user management](06-user-management.md)); and tenant
provisioning, deprovisioning, suspension, and residency-aware **placement**
([13 tenant lifecycle](13-tenant-lifecycle.md)).
...
2. **Dual-control gate.** Erasure is destructive and touches PII, so it runs as a non-cascading `data_export`/`iam_change`-class capability with proposer not equal to approver, gated and audited through the proposal workflow ([12 Admin API](12-admin-api.md), [05 authorization](05-authorization.md)). Never autonomous.
...
- **Rectification (Art.16).** Identity and profile data is mutable and is corrected
  ...
  [06 user management](06-user-management.md) (dual-control for sensitive fields,
...
- **Objection (Art.21).** Core authentication is contract or legal-obligation based,
  ...
  ([07 email](07-email-notification.md)); any legitimate-interest balancing is routed to
  the DPO.

### Consent receipts (Art.7(1))

On granting consent, [08 UI](08-login-consent-ui.md) emits an immutable, hash-chained
...
in-jurisdiction (Silo pinning) is owned by [13 tenant lifecycle](13-tenant-lifecycle.md);
...
timestamps) live in [02 data](02-data.md)/[12 Admin API](12-admin-api.md); the
`SuppressionEntry` table reused for objection lives in [07 email](07-email-notification.md).
...
- **Erasure is never automatic on offboard.** Offboard invokes the gated erasure saga, but erasure stays dual-control and Art.17/DPO-gated ([06 user management](06-user-management.md)); the saga can revoke and disable without erasing until the gate clears.
...
- There is no global super-admin; a platform-level erasure operator is still a scoped or global-role grant anchored to a root tenant ([05 authorization](05-authorization.md)).
...
- Design docs: [02 data](02-data.md) (`SubjectDek`, `AuditLog`, `ProcessingRestriction` reference), [03 audit](03-audit.md) (`IAuditChainScrubber`, `RecordHash`), [12 Admin API](12-admin-api.md) (dual-control frame), [09 key management](09-key-management.md) (key destroy), [06 user management](06-user-management.md) (rectification surface), [07 email](07-email-notification.md) (suppression, priority lane), [13 tenant lifecycle](13-tenant-lifecycle.md) (residency placement, deprovision).
- [Architecture](../architecture/README.md); [Pre-GA ratification checklist](../PRE-GA-RATIFICATION-CHECKLIST.md).

---

Previous: [12 Admin App](12-admin-app.md) · Next: [13 Tenant lifecycle](13-tenant-lifecycle.md)
