...
| `auth_time` | JSON **number**, a NumberDate, via the `long` overload | id_token and access_token | Stamped as an **immutable claim** at sign-in, in the same `SignInWithClaimsAsync` call as `amr` (08). Deliberately **not** `AuthenticationProperties.IssuedUtc`, which a sliding cookie re-issues on ordinary traffic | `max_age` and step-up freshness |
