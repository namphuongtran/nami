...
- **Cross-doc consistency (with [09](12-key-management.md))**: the keys-health gauge names owned by 12 (`key_rotations`, `keys_loaded`, `signing_key_days_to_expiry`) do not carry the `nami.`-rooted prefix that the telemetry-naming rule (ADR-0065) requires, and should, for full contract conformance; raised here, to be reconciled when 09 is next revised, rather than renamed across a committed doc from here.
