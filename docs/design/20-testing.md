...
| **Every dual-control action in the ADR-0081 catalogue is reachable**: for each `ActionType`, assert some endpoint raises it. This is a reachability assertion rather than a behaviour one, and it exists because the two defects fixed on 2026-08-01 were `secret-revoke` and `audit-export` sitting fully specified in the target-guard table with **no route to raise them from**. No functional test notices that, because there is nothing to call, and ADR-0079 rule 5 means no generic route can stand in. Plus the two routes themselves: `DELETE /applications/{id}/secrets/{secretId}` requires `If-Match` and revokes one credential out of a multi-key set rather than the client's whole credential state, and `POST /audit/export` goes direct under the threshold and answers `202` with a proposal id over it | [15](15-admin-api.md), [07](07-authorization.md) |
