...
| No-restart key rotation, as a **contract** on the seam rather than on our own code: signing uses the active key while the JWKS still carries announced, active and retired; a token signed with the new key self-validates with no restart; reading `CurrentValue` repeatedly does **not** mint a key each time; and the whole contract re-runs on **every** OpenIddict bump, so a seam that stops holding fails the build instead of silently breaking self-validation in production | [12](12-key-management.md), [22](22-openiddict-seam-catalogue.md) |
