...
**The `xmin` convention has one trap, and it is the whole reason the DDL above never
declares it: declaring the property is right, declaring the column is the bug.** `xmin` is a
PostgreSQL **system** column that exists on every table and is never named in `CREATE
TABLE`. A design that declares it (`"RowVersion" xid NOT NULL`, or worse a `bytea`, which is
the SQL-Server `rowversion` shape ADR-0036 rejects) creates a real column that **nothing
writes**, so its value never changes. The whole chain then fails silently: the ETag never
changes, `If-Match` always matches, `DbUpdateConcurrencyException` never fires, two admins
editing one row lose an edit with no error, and the dual-control `TargetETag` re-check
([15](15-admin-api.md) section 3.6) compares a value that **cannot** differ, so the
`mutate`-class TOCTOU guard is vacuous for that table across the whole 72-hour proposal
window.

The trap also arrives from the code side: EF Core emits a column for a mapped property
unless it is excluded from the migration, so getting the property right and the migration
wrong reproduces the same defect. On the pinned stack the property is marked as a row
version and the provider's convention binds it to the system column; **the exact binding is
a verify-at-build item**. What is settled is the negative: `UseXminAsConcurrencyToken()`,
the API older material names for this, **does not exist on the pinned provider**, verified
by its absence from both the XML documentation and the assembly of
`Npgsql.EntityFrameworkCore.PostgreSQL` 10.0.2, which also carries no `xmin` string at all.
The convention that replaced it is present in that assembly as `ProcessRowVersionProperty`
alongside `SystemColumnNames` and `IsSystemColumn`. A migration test asserting that **no
table has a physical `RowVersion` column** is the cheap guard, because this defect is
