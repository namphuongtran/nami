...
([13 erasure and DSR](13-erasure-and-data-subject-rights.md)); the key-set bootstrap,
escrow, destruction, and the DP-key delete-is-irreversible caveat
([09 key management](09-key-management.md)); the dual-control proposal workflow and the
`PUT /tenants` guards and problem codes ([12 Admin API](12-admin-api.md)); the
...
runs under dual-control ([12 Admin API](12-admin-api.md), [05 authorization](05-authorization.md)).
...
3. Erase or archive the subject data through the erasure saga ([13 erasure and DSR](13-erasure-and-data-subject-rights.md)).
4. **Escrow then destroy** the key-set: short-term escrow per the retention window, then destroy; not an immediate destroy, and the destroy distinguishes a soft-delete under purge-protection from an irreversible hard-delete of a DP key ([09 key management](09-key-management.md)).
...
[13 erasure and DSR](13-erasure-and-data-subject-rights.md).
...
lifecycle executors read/write live in [02 data](02-data.md)/[12 Admin API](12-admin-api.md).
...
- Design docs: [02 data](02-data.md) (`Tenants`, `TenantClosure`, RLS, `SchemaVersion`), [13 erasure and DSR](13-erasure-and-data-subject-rights.md) (the erasure saga, transfer register, jurisdiction profile), [09 key management](09-key-management.md) (key bootstrap, escrow, destroy), [12 Admin API](12-admin-api.md) (the `PUT /tenants` guards, suspend/resume, provision endpoints), [04 core protocol](04-core-protocol.md) (per-tenant issuer from `Identifier`), [03 audit](03-audit.md) (audit-forward residency assertion).
