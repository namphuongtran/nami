...
can read it. Claims are deny-by-default by destination. Every issuance writes one
...
|---|---|---|
| End-user step-up authentication | `max_age` / `prompt` / `acr_values` to an MFA challenge, then updated `acr` / `amr`. Its architectural content is the assurance model, not the sequence | ADR-0013 for the model, [05-authorization](../design/05-authorization.md) for the gate, [08-login-consent-ui](../design/08-login-consent-ui.md) for the sequence. Flow 2 shows the admin-side challenge |
| Device authorization | Server-enforced `interval` and `slow_down` polling backoff with a 429 ceiling. Native grant, no distributed interaction | [11-advanced-flows](../design/11-advanced-flows.md), ADR-0014 |
| Pushed authorization request | Native, enabled per client. The architectural content is the `request_uri` lifetime (shortened to 5 to 600 seconds) and the per-client anti-flood cap | [11-advanced-flows](../design/11-advanced-flows.md), ADR-0014 |
...
| Silo migration fan-out | Ordered ring rollout with a per-tenant 503 traffic gate. An operational rollout, drawn where the schema lives | [02-data](../design/02-data.md), [13-tenant-lifecycle](../design/13-tenant-lifecycle.md), ADR-0017 |
| Tenant deprovision and re-home | The ordered inverse of flow 7, plus key escrow-then-destroy and old-scope invisibility checks | ADR-0017, [13-tenant-lifecycle](../design/13-tenant-lifecycle.md) |
...
* Flow 2: ADR-0020 (dual control enforced server-side, app-only tokens rejected),
  ADR-0013 (the assurance producer), [05-authorization](../design/05-authorization.md)
  (`RequireActor`, the `request_hash` binding, the `max(client, scope, runtime)` rule),
  and [12-admin-api](../design/12-admin-api.md) (the 72-hour proposal expiry and the
  `TargetETag` guard).
* Flow 3: ADR-0011 (the custom options monitor, the 90/14/14 shape, the local-validation
  ...
  guards), and [09-key-management](../design/09-key-management.md) (the default values,
...
* Flow 5: ADR-0014 (DPoP as a build, spikes A-1 and A-3), ADR-0048 (introspection
  ...
  [11-advanced-flows](../design/11-advanced-flows.md) (the `ath` asymmetry and refresh
  binding).
...
* Flow 9: ADR-0010 (initiator-based authority, `IsInheritable`, the additive v1 grant
  ...
  port), [05-authorization](../design/05-authorization.md) (the scoped handler, the
...
* Flow 12: ADR-0004 (`SetAuthorizationId`, the deliberately unbounded consent lifetime),
  ...
  [08-login-consent-ui](../design/08-login-consent-ui.md) (the consent page and receipt
  emission).
...
* Flow 15: ADR-0014 (the native grant with Nami-owned `act` resolution), ADR-0010
  ...
  [05-authorization](../design/05-authorization.md) (the three-way initiator
  classification and the `may_act` exclusion),
  [11-advanced-flows](../design/11-advanced-flows.md) (the grant wiring boundary).
...
* Reconciled against the design corpus's runtime and flow view on 2026-07-25. Six flows
  ...
  [05-authorization](../design/05-authorization.md) excludes it deliberately; and its key
