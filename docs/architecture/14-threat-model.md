...
| D3 | Account lockout weaponised against a victim | A3 | **Per-source failure scoping alongside per-account lockout**, so fail-spam from one source cannot lock a chosen account, with a distinct alert for many lockouts on one account (ADR-0042). That alert is produced by the built-in detector grouping on `SubjectRef` in the audit lane (ADR-0083); until that mechanism existed this mitigation named an alert no lane could generate (ADR-0082) | Medium |
...
| E1a | Delegated administrator **minting** authority they do not hold, including to themselves, or building a re-delegation chain | A4 | A **grant-time ceiling** is the primary control: every capability in a requested grant must already be held by the actor at the route tenant, refused `403` rather than queued for approval, plus an expiry ceiling so a grant cannot outlive its parent. Dual-control keyed on **reach** is supplementary above it, and the `re_delegate` gate is direct-only. Residual, accepted: the ceiling is evaluated at grant time, so narrowing a delegator's own grant does not retroactively narrow grants they already issued (ADR-0010) | High |
