...
| [0096](../adr/0096-fluent-builder-api-surface.md) | Fix the fluent-builder API surface, binding `NamiIdentityOptions` from configuration before the delegate runs, keeping `INamiIdentityBuilder` to a service-collection carrier, and failing at start-up rather than marking the two required options `required` | NONE |
