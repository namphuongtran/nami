...
| R7 | **Back-channel logout is an interim build**, and front-channel logout is dead under third-party-cookie blocking | M | M | Built now on the session store that already exists, with a decommission marker for the native equivalent. A relying party without a back-channel endpoint degrades to bounded logout at the access-token lifetime, which is a **stated parity boundary** (ADR-0019). That lifetime is the real bound only because the refresh grant is gated on the session still existing (ADR-0003, executed in [core protocol](../design/04-core-protocol.md)); remove the gate and the degradation runs to the 8-hour refresh ceiling | Maintainers |
