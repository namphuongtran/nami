...
| A literal `<TargetFramework>net10.0</TargetFramework>` | `$(NamiLibraryTargetFrameworks)`, or `$(NamiApplicationTargetFramework)` for an application | `src/CLAUDE.md:156`, ADR-0030:39 |
...
2. **`AnalysisMode` is `Recommended`, and `CA1819` is not in it.** Public array-returning members
   produce no warning today. Do not read a green build as approval of one. `src/CLAUDE.md:184`,
   ADR-0094.
...
| Public API surface | ADR-0044; a public type is two files (`src/CLAUDE.md:93-123`) |
...
The heavier .NET skills earn their place as `src/` fills, and it holds **six** public types today
(four sealed classes and two interfaces, counted 2026-08-05). Reach for
