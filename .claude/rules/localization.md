...
| Moving `UseRequestLocalization` earlier, so more of the pipeline sees the culture | Its position is fixed: after security headers and CSP, before routing. ADR-0091:123 depends on that order and says so | design 11 section 6.1, ADR-0091:123 |
...
**2. The `en` floor makes a missing translation look like success.** Design 11 section 5.6 and its
acceptance line in section 9 both say a missing key falls to the `en` floor and warns **once**. So
an entirely absent culture serves an English page and emits a single log line. The acceptance
invariant tests that the fallback happens, which is the opposite of noticing in production that it
happened. Whatever detects a half-translated deployment is not in this repository, and no source
here says what it should be.
...
1. **The name scrub and the docs guardrail are both scoped to markdown.** This one is firm. The
   hook's name scrub reads only the **staged** `*.md` files
   (`git diff --cached --name-only --diff-filter=ACM -- '*.md'`, `scripts/hooks/pre-commit:25`),
   and guardrail Check 1 reads `git ls-files '*.md'` (`scripts/check-adrs.sh:28`). The root
   ...
   product comparison. No gate sees it. [`razor.md`](razor.md) records the same hole for a
   `.cshtml`.
2. **Whether an analyzer reads a `.resx` string value is settled by no source here.** This row is
   the weakest in this file, and it is kept because assuming the opposite is worse. ADR-0092:147
   says "The SDK analyzers see C#", then names four things they do not see: "Razor markup, SQL
   held outside C#, Dockerfiles, or GitHub Actions workflow definitions". A `.resx` is **not**
   among the four, and `resx` returned zero hits in that ADR, measured 2026-08-07. So this row
...
translation into a language whose typography uses one. Raise it rather than assume either answer.
...
file, in **substring** form. The substring form over-counts and never under-counts, so a zero is
reliable. Do not re-run these with `\b`: `docs/CLAUDE.md` records that `git grep -E` ignores it
here and returns zero for every term.
...
| Email templating, the template chain, and the ban on Razor | ADR-0038 parameter G, and design 10 section 5.4 |
| Whether a configuration key may be renamed | ADR-0044 parameter I |
| The admin console surface | ADR-0020, with `docs/design/16-admin-app.md` |
